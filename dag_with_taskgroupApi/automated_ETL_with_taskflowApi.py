...
def postgres_load_s3(data_interval_start, data_interval_end):
    ...
    # sql = """ 
    #      select * from stg_orders where orderdate >= {data_interval_start} and orderdate < {data_interval_end}
    # """    
    #cursor.execute(sql)
    with NamedTemporaryFile(mode='w',suffix=f"{data_interval_start}" ) as f:
       csv_writer = csv.writer(f)
       csv_writer.writerow([i[0] for i in cursor.descriptions])
...
with DAG(
    dag_id='automated_ETL_with_taskflowAPI_v01',
    ...
    with TaskGroup("postgres_load_s3", tooltip="Extract staged data and load to S3 bucket ") as load_postgres_to_s3:
        ...
        date_end = '1997-11-26'
        postgres_to_s3 = postgres_load_s3(date_start, date_end)
        transform_src_order = transform_src_orders()
        # task order
    ...
    extract_load_src >> transform_src_tabe >> load_customer_order_model  
    extract_load_src >> transform_src_tabe >> load_postgres_to_s3     
import time
from datetime import datetime, timedelta
import logging
import csv
